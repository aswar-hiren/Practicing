...
namespace LogicLayer.Repo_admin
{
  ...
  .ToList();
        ...
        {
            return _context.Physicians.Where(ph => ph.Regionid == region && ph.Isdeleted == new System.Collections.BitArray(new[] { false })).ToList();
        }
        public void deletePhy(int phyid)
        {
            var phy = _context.Physicians.Where(ph => ph.Physicianid == phyid).FirstOrDefault();
            phy.Isdeleted = new System.Collections.BitArray(new[] { true });
            _context.SaveChanges();
        }
        ...
        public CloseCaseViewModel getclosedetails(int reqid)
        {
            ...
            var reqclient = _context.Requestclients.Include(rc=>rc.Request).Where(rc => rc.Requestid == reqid).FirstOrDefault();
            closeCaseViewModel.Encounter = _context.Encounters.FirstOrDefault(en => en.Requestid == reqid);
            ...
            closeCaseViewModel.status = reqclient.Request.Status;
        ...
        public Providervm getPhysicianData(int regionId)
        {
            if (regionId == 0)
            {
                ...
                providervm.physicians = _context.Physiciannotifications.Include(ph => ph.Physician).Include(ph => ph.Physician.Role).Where(ph=>ph.Physician.Isdeleted == new System.Collections.BitArray(new[] { false })).ToList();
                return providervm;
            }
            else
            {
                Providervm providervm = new Providervm();
                providervm.physicians = _context.Physiciannotifications.Include(ph => ph.Physician).Include(ph => ph.Physician.Role).Where(ph => ph.Physician.Regionid == regionId && ph.Physician.Isdeleted == new System.Collections.BitArray(new[] { false })).ToList();
                return providervm;
            }
        }
        ...
        public List<Physician> MdStatusDetails(int regionId)
        {
            var data = _context.Physicians
                       .Include(a => a.Shifts)
                       .ThenInclude(sd => sd.Shiftdetails).Where(a => (a.Regionid == regionId || regionId == 0) && a.Isdeleted == new System.Collections.BitArray(new[] { false }))
                       .ToList();
        ...
        public AccessPagevm GetAccessPagevm()
        {
            ...
            {
                roles = _context.Roles.Where(role=>role.Isdeleted == new System.Collections.BitArray(new[] { false })).ToList(),
            };
            return accessPage;
        }
        ...
        {
            var role = _context.Roles.Where(ph => ph.Roleid == roleid).FirstOrDefault();
            role.Isdeleted = new System.Collections.BitArray(new[] { true });
            _context.SaveChanges();
        ...
        {
            if (regionid == 0)
                return _context.Physicians.Where(ph=> ph.Isdeleted == new System.Collections.BitArray(new[] { false })).ToList();
            else
            {
                return _context.Physicians.Where(ph => ph.Regionid == regionid && ph.Isdeleted== new System.Collections.BitArray(new[] { false })).ToList();
            }
        ...
        {
            var physicianRegion = _context.Physicians.Where(m => m.Regionid == RegionId && m.Isdeleted== new System.Collections.BitArray(new[] { false })).Select(m => new JObject() { { "PhysicianId", m.Physicianid }, { "PhysicianName", m.Firstname } }).ToList();
